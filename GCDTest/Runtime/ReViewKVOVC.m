...
@interface ReViewKVOVC ()
@property (strong, nonatomic)   UIButton  *testBtn;  /**< 打印Btn */
@property (strong, nonatomic)   CC  *C;  /** C **/
@end

@implementation ReViewKVOVC
...
- (void)viewDidLoad {
    ...
    [_C addObserver:self forKeyPath:@"age" options:NSKeyValueObservingOptionOld|NSKeyValueObservingOptionNew context:nil];
...
//    STLog(@"%@",keyPath);
//    STLog(@"%@",object == _C?@"YES":@"NO");

    // 判断是否为_B的属性“age”:
    if([keyPath isEqualToString:@"age"] && object == _C) {
        // 响应变化处理：UI更新（label文本改变）
        STLog(@"%@",[NSString stringWithFormat:@"当前的num值为：%@",
                     [change valueForKey:@"new"]]);
        //change的使用：上文注册时，枚举为2个，因此可以提取change字典中的新、旧值的这两个方法
...
-(void)dealloc{
    /* 3.移除KVO */
    [_C removeObserver:self forKeyPath:@"age" context:nil];
}
